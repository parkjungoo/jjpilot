...
#define DISARM_DELAY            5  // called at 10hz so 2 seconds
...
void Copter::arm_motors_check()
{
    ...
    // full left
        ...
        if (arming_counter == DISARM_DELAY && motors->armed()) {
            init_disarm_motors();
			hal.uartC->printf("#2#");		// my_source
			m_rcInfo.mode = MY_SOURCE_MODE_DISARMING;	// my_source
			my_source_init_base_alt();		// my_source
        }
...
void Copter::auto_disarm_check()
{
	...
	if( m_rcInfo.mode == MY_SOURCE_MODE_FAILSAFE )		// my_source
		disarm_delay_ms = 1;
    ...
    if ((tnow_ms-auto_disarm_begin) >= disarm_delay_ms) {
        init_disarm_motors();
        auto_disarm_begin = tnow_ms;
		hal.uartC->printf("#2#");		// my_source auto-disarming
		m_rcInfo.mode = MY_SOURCE_MODE_DISARMING;	// my_source
		my_source_init_base_alt();		// my_source
    }
}
...
bool Copter::init_arm_motors(bool arming_from_gcs)
{
    ...
    // return success
	hal.uartC->printf("#1#");	// my_source	arming
	m_rcInfo.mode = MY_SOURCE_MODE_ARMING;
    return true;
}
