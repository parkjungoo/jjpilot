...
#include <pthread.h>	// my_source
#include "MyQueue.h"	// my_source
...
// start my_source
#define MY_SOURCE_MAKE_STRING(X)        "#"#X"#"
#define MY_SOURCE_MAKE_MODE_MSG(X)      MY_SOURCE_MAKE_STRING(VAL)
#define MY_SOURCE_GET_MODE_MSG(X)       (('0') + (X))

#define MY_SOURCE_STR_LEN				100
#define MY_SOURCE_PWM_LEN				10
#define MY_SOURCE_COMMAND_PWM           '@'
#define MY_SOURCE_COMMAND_MODE          '#'

#define MY_SOURCE_MODE_DISABLE_ARM      0      // initial state        #-1#    must not send and rcv
#define MY_SOURCE_MODE_ARMING           1       // Arming               #1#     send msg
#define MY_SOURCE_MODE_DISARMING        2       // Disarming            #2#     send msg
//#define MY_SOURCE_MODE_SAFETY_TOGGLE    3       // Safety toggle        #3#
//#define MY_SOURCE_MODE_CAL_PWM			4
#define MY_SOURCE_MODE_UP_KP			4
#define MY_SOURCE_MODE_DOWN_KP			5
#define MY_SOURCE_MODE_UP_ACZ			6
#define MY_SOURCE_MODE_DOWN_ACZ			7
#define MY_SOURCE_MODE_FAILSAFE			8		// disconnect bluetooth.
#define MY_SOURCE_MODE_ENABLE_ARM       9       // bluetooth connect    #0#     rcv msg
#define MY_SOURCE_MODE_STABILIZE        11       //  ?
#define MY_SOURCE_MODE_ALT_HOLD         12       //  ?
#define MY_SOURCE_MODE_AUTO_TUNE        13       //  ?
#define MY_SOURCE_MODE_AUTO_LANDING		14

#define MY_SOURCE_MODE_UP_ACZ_ALTHOLD		21
#define MY_SOURCE_MODE_DOWN_ACZ_ALTHOLD		22
#define MY_SOURCE_MODE_UP_ALT_ALTHOLD		23
#define MY_SOURCE_MODE_DOWN_ALT_ALTHOLD		24
#define MY_SOURCE_MODE_UP_ALPHA_ALTHOLD		25
#define MY_SOURCE_MODE_DOWN_ALPHA_ALTHOLD	26


#define MY_SOURCE_CH_CRD_YAW			0
#define MY_SOURCE_CH_CRD_THROTTLE		1
#define MY_SOURCE_CH_CRD_ROLL			2			
#define MY_SOURCE_CH_CRD_PITCH			3
#define MY_SOURCE_CH_CRD_LASER			4

#define MY_SOURCE_CH_PWM_YAW			3
#define MY_SOURCE_CH_PWM_THROTTLE		2
#define MY_SOURCE_CH_PWM_ROLL			0
#define MY_SOURCE_CH_PWM_PITCH			1
#define MY_SOURCE_CH_PWM_LASER			8

#define MY_SOURCE_CRD_MAX				500
#define MY_SOURCE_CRD_MIN				-500

#define MY_SOURCE_PWM_ON_LASER			20000
#define MY_SOURCE_PWM_OFF_LASER			0

#define MY_SOURCE_OFFSET_THROTTLE		2
#define MY_SOURCE_OFFSET_YAW			3
#define MY_SOURCE_OFFSET_ROLL			0
#define MY_SOURCE_OFFSET_PITCH			105

    // func
	void userhook_200Hz();
	void userhook_20Hz();
	void my_source_sort_command(char * strCommand);
	void my_source_rc_init();
	void my_source_mode_select(char * mode);
	bool my_source_is_this_arm(void);
	bool my_source_is_this_disarm(void);
	void my_source_calc_sigma(void);
	void my_source_read_cds(void);
	void my_source_check_hit(void);
	void my_source_check_hit1(void);
	void my_source_check_hit2(void);
	void my_source_fail_safe(void);
	void my_source_auto_landing(void);		// auto_landing
	void my_source_rc_cal_pwm(void);
	void my_source_display_ins(void);
	void my_source_update_accel(void);
	void my_source_set_base_accel(void);
	void my_source_set_base_alt(void);
	void my_source_init_base_alt(void);
	void my_source_alt_hold(void);			// alt_hold

	// val
	MyQueue m_Queue;
	pthread_mutex_t m_qMutex;

	typedef struct __my_source_rc_info{	
		int16_t mode;   // -1:disable_arm, 0:enable_arm, 1:arming, 2:disarming
		int16_t state;	// 3:auto-landing
		uint16_t pwm[MY_SOURCE_PWM_LEN];
		int16_t coordinates[5];
		bool laserFlag;
		uint16_t laserCount;
		...
		double set_AcZ;
		int16_t my_pterm;
		int16_t autoLandingFlag;
		double Base_Accel_Z;
		double Base_Alt;
		uint8_t baseAltCount;
		uint8_t endAltFlag;
		double altHold_kpAcZ;
		double altHold_kpAlt;
		double altHold_alpha;
	}MySourceRcInfo;

	typedef struct __my_source_adc_laser{
		uint16_t cnt;
		uint32_t sig;
		uint32_t brt_sum;
		uint32_t brt_sum_sqr;
		double sigma;
		uint32_t sigma_prev;
		uint32_t t1;
		bool	hit;
	...
	MySourceRcInfo m_rcInfo;
	MySourceAdcLaser m_adcLaser;

	char m_str[MY_SOURCE_STR_LEN];
	int m_strLen =0;
	bool m_bStrFlag = false;
