...
public:
	...
	// my_source
	void my_source_lib_rc_init(void);
	int my_source_lib_rc_orb_check(int handle, bool * updated);
	void my_source_lib_rc_update_pwm(uint16_t values, int ch);
	void my_source_lib_rc_display(void);
	void my_source_lib_rc_set_pwm(void);

private:
	// my_source
	bool m_bFlag = false;
	uint16_t m_uI16Values[10];
